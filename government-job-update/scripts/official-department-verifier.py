...
def authority_hit(title,desc,url):
 ...
 return any(re.search(r'(?<![a-z])'+re.escape(alias)+r'(?![a-z])',hay,re.I) and any(part in host for part in re.findall(r'[a-z]{4,}',alias.lower())) for alias in AUTHORITY_ALIASES)
...
def rich_fields(item,source_text,source_title=''):
 ...
 r=fields.normalize_record(base); r['title']=item.get('title','') or source_title or r.get('title',''); r['source']=item.get('discoverySource','MultiSource'); r['category']=base['category']; r['updateType']=base['category']
 return r
...
def verify_one(item):
 ...
 candidates=[]; candidates.extend(authority_candidates(title,desc)); candidates.extend([u for u in links(discovery_body,item.get('url','')) if official(u)])
 ...
 for resolved,page,ot,txt in pages:
  ...
  # A government domain alone is never sufficient. Require meaningful title overlap or a trusted authority explicitly named in the discovery record.
  if year and signal and ((overlap>=2 and sc>=0.30) or (authority_hit(title,desc,resolved) and overlap>=1 and sc>=0.30)):
   found.append((sc,overlap,resolved,ot,txt))
 if found:
  sc,overlap,url,ot,txt=max(found,key=lambda x:(x[0],x[1])); result['officialSource']={'sourceId':'official-government-domain','url':url,'title':ot,'matchScore':sc,'titleOverlap':overlap}
  # If the official notice is richer than the discovery page, merge useful fields without losing source-page details.
 ...
 result['checks']['nonempty_title']=bool(title); result['checks']['discovery_signal']=bool(title or desc); result['checks']['discovery_fetched']=bool(discovery_text)
 result['checks']['official_notice_url']=bool(result['officialSource']); result['checks']['trusted_source']=bool(result['officialSource']); result['checks']['safe_http_urls']=bool(result['officialSource'] and result['officialSource']['url'].startswith(('http://','https://'))); result['checks']['official_content_match']=bool(result['officialSource'] and result['officialSource']['matchScore']>=0.30)
...
def main():
 ...
 save(CAN,canonical); save(OUT,{'schemaVersion':7,'checkedAt':now,'sourceStatus':state.get('status'),'counts':counts,'items':results}); save(LOG,{'checkedAt':now,'status':'ok','counts':counts,'strategy':'Parallel official verification with strict title/source matching and rich source-page field extraction; no blind verification reuse'}); return 0

...
def rss_links(b):
 ...
 return out
def html_links(b):
 s=b.decode('utf-8','replace'); out=[]
 for href in re.findall(r'href=[\"\']([^\"\']+)[\"\']',s,re.I):
  u=urljoin(BASE,href)
  if u.startswith(BASE) and u.split('#',1)[0] != BASE: out.append(u.split('#',1)[0])
...
def main():
 ...
 if c==200:
  r=rss_links(x)
  if r: urls += r; log['discoveryChannels'].append('public-rss')
 c,x,final,e=get(urljoin(BASE,'sitemap.xml'))
 if c==200:
  parsed=False
  try:
   root=ET.fromstring(x); kind=root.tag.rsplit('}',1)[-1]; first=locs(x); parsed=True
   if kind=='sitemapindex':
    for child in first[:5]:
     cc,xx,ff,ee=get(child)
     if cc==200: urls += locs(xx)
   elif kind=='urlset': urls += first
  except Exception: pass
  if parsed and urls: log['discoveryChannels'].append('public-sitemap')
  elif final.rstrip('/') == BASE.rstrip('/'):
   h=html_links(x)
   if h: urls += h; log['discoveryChannels'].append('public-sitemap-redirect-html')
 if not urls:

...
ROOT=Path(__file__).resolve().parents[1];VERIFICATION=ROOT/'data'/'official-verification-state.json';CANONICAL=ROOT/'data'/'canonical-job-records.json';MONITOR=ROOT/'data'/'sarkariresult-monitor-state.json';OUT=ROOT/'data'/'publication-queue.json';REQUIRED_FIELDS=('title','notificationUrl','source','verificationStatus','publicationStatus')
def main():
 ...
 result={'schemaVersion':4,'generatedAt':verification.get('checkedAt'),'policy':'verified-only','readyCount':len(queue),'blockedCount':blocked,'categoryCounts':category_counts,'items':queue}

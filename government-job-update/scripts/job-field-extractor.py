...
FIELD_LABELS={'department':('department','ministry','organization/department','department name'),'vacancies':('total vacancies','number of vacancies','vacancy','vacancies','posts'),'qualification':('educational qualification','qualification','eligibility'),'ageLimit':('age limit','maximum age','upper age','age'),'applicationStartDate':('application start date','starting date','start date','online application starts'),'applicationLastDate':('last date','closing date','last date to apply','application last date'),'examDate':('exam date','examination date','date of examination')}
CATEGORY_RULES=(
...
def normalized_date(value):
    ...
    for p in DATE_PATTERNS:
        ...
        if m:
            try:
                if len(m.groups())==3 and m.group(2).isdigit(): d,mo,y=map(int,m.groups()); return datetime(y,mo,d).strftime('%Y-%m-%d')
...
def extract_after_label(text,labels,max_len=500):
    lines=[clean(x) for x in text.splitlines() if clean(x)]; low=[x.lower() for x in lines]
    for label in labels:
        for i,line in enumerate(low):
            if label in line:
                same=re.split(re.escape(label),lines[i],flags=re.I,maxsplit=1); value=clean(same[1] if len(same)>1 else '')

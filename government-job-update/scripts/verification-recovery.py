...
def get(url):
    try:
        ...
        with urllib.request.urlopen(req, timeout=TIMEOUT) as r:
            return r.status, r.read(1200000), r.geturl()
    except Exception:
        return None, b'', url
...
def links(body, base):
    ...
    for href in re.findall(r'href=["\']([^"\']+)["\']', html, re.I):
        ...
        if not u.startswith(('http://', 'https://')) or u in out:
            continue
        low = u.lower()
        if official(u) and (useful_path(u) or any(k in low for k in ('recruit', 'career', 'vacan', 'notice', 'notification', 'advert', 'job', 'result', 'admit', 'answer', 'syllabus', 'admission', 'apply'))):
            out.append(u)
...
def candidate_score(job_title, page_title, page_text):
    a = tokens(job_title)
    b = tokens(page_title + ' ' + page_text[:30000])
    overlap = len(a & b)
...
def recover_one(item_id, item):
    ...
    with ThreadPoolExecutor(max_workers=min(10, len(candidates))) as pool:
        ...
        for f in as_completed(futures):
            try:
                ps, page, final = f.result()
            except Exception:
                continue
            if ps == 200 and page and official(final):
                pages.append((final, page, title(page), text(page)))
    best = None
    job_title = item.get('fields', {}).get('title') or item.get('title') or ''
    for final, page, page_title, page_text in pages:
        ...
        if overlap >= 1 and (category_signal or year_signal):
            ...
            if best is None or score > best[0]:
                best = (score, overlap, final, page_title, page_text)
    if not best:
        return None

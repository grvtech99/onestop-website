...
function render(cat,items){
 ...
 var visible=items.slice(0,15);
 ul.innerHTML=visible.map(function(x){return '<li><span class="fresh-badge fresh-new">VERIFIED</span><a href="#" class="item os-api-item" data-id="'+esc(x.id)+'">'+esc(x.title)+'</a><small style="display:block;color:#60708d;font-size:11px;margin-top:3px">'+esc(x.meta||x.dates)+'</small></li>';}).join('');
 visible.forEach(function(x){var all=ul.querySelectorAll('[data-id]');for(var i=0;i<all.length;i++){if(all[i].getAttribute('data-id')===x.id){all[i].addEventListener('click',function(e){e.preventDefault();detail(x);});break;}}});
}
function addStatus(){var notice=document.querySelector('.notice');if(!notice)return;notice.innerHTML='<strong>✅ Verification:</strong> This page displays only records released through the verified-only publication queue. Unverified or held records are never rendered here. Always review the official notice before applying.';}
function load(){
 ...
 fetch(API_URL,{cache:'no-store',headers:{'Accept':'application/json'}}).then(function(r){if(!r.ok)throw new Error('API HTTP '+r.status);return r.json();}).then(function(payload){
   ...
   var hero=document.querySelector('.hero p');if(hero)hero.textContent=items.length?'Verified government updates — '+items.length+' publication-ready record'+(items.length===1?'':'s')+' currently available.':'No verified publication-ready government updates are available right now.';
 }).catch(function(err){

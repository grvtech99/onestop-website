...
(function () {
  ...
  function render(list){
    ...
    if(listMode || selected!=='all') {root.innerHTML=filtered.length?filtered.map(card).join(''):'<div class=\"empty\">अभी कोई प्रकाशित लेख नहीं मिला।</div>';return;}
    ...
    let html=section('Latest Articles',filtered.slice(0,6),'all');
    groups.forEach(([name,key])=>{const items=filtered.filter(a=>String(a.category||'').toLowerCase()===key.toLowerCase()).slice(0,4);html+=section(name,items,key);});

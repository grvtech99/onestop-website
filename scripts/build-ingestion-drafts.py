    ...
    for p in sorted(CAND.glob('*.json')):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            if r.returncode!=0:
                ...
                if trace: item.update({'contentSha256':trace.group(1),'contentType':trace.group(2),'extractedChars':int(trace.group(3))})
                ...
                if r2.returncode==0 and out.exists():
                    draft=json.loads(out.read_text(encoding='utf-8')); draft.update({'sourceId':item.get('sourceId'),'sourceName':item.get('sourceName'),'noticeUrl':url,'applyUrl':url,'contentSha256':item.get('contentSha256',''),'contentType':item.get('contentType',''),'extractedChars':item.get('extractedChars',0),'discoveryPriority':item.get('discoveryPriority','high'),'reviewStatus':'draft','verificationRequired':True,'verificationNote':'Automatically extracted candidate. Verify every field against the official notice before publication.'})

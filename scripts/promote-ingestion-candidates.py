...
DRAFT=ROOT/'data/government-ingestion-drafts'; QUEUE=ROOT/'data/government-update-review-queue.json'; PUBLIC=ROOT/'onestop-updates-data.js'
def key(item):
...
def main():
    ...
    existing={x.get('key') for x in q.get('items',[])}; public=PUBLIC.read_text(encoding='utf-8') if PUBLIC.exists() else ''; now=datetime.now(timezone.utc).isoformat(); added=0

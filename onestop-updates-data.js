/* ONESTOP Government Updates — official-source data layer
   ...
   Detail fields intentionally use "Official notice में देखें" when the current feed
   does not expose a verified value, so ONESTOP never invents recruitment details.
*/
(function(){
'use strict';
...
function openDetail(x){
 ...
 document.getElementById('detailMeta').textContent=x.meta||'';
 var fields=[['Post / Vacancy',x.vacancy],['Important Dates',x.dates],['Eligibility',x.eligibility],['Application Fee',x.fee],['Age Limit',x.age],['Selection Process',x.selection]];
 ...
 var notice=document.getElementById('officialNotice'),apply=document.getElementById('applyLink');
 notice.href=x.noticeUrl||x.url;apply.href=x.applyUrl||x.url;
...
function run(){
 render('jobs',DATA.jobs);render('admit',DATA.admit);render('results',DATA.results);render('answer',DATA.answer);render('admission',DATA.admission);render('scholarship',DATA.scholarship);render('syllabus',DATA.syllabus);render('important',DATA.important);
 Object.keys(DATA).forEach(function(id){DATA[id].forEach(function(x,i){var el=document.querySelector('[data-update-id="'+id+'-'+i+'"]');if(el)el.addEventListener('click',function(e){e.preventDefault();openDetail(x);});});});
 var close=document.getElementById('closeModal');if(close)close.addEventListener('click',closeDetail);
 var modal=document.getElementById('updateModal');if(modal)modal.addEventListener('click',function(e){if(e.target===modal)closeDetail();});
